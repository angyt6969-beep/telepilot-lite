...
export const FREE_TRIAL_BUTTON_TEXT = "Claim your Free 1 day key!";
...
async function handleFreeTrialHttp(req, res) {
  ...
  if (req.method === "GET" && url.pathname === "/free-trial/session") {
    ...
    const tutorial = readFreeTrialTutorial(session.uid);
    const subject = freeTrialSubject(session.uid);
    const existing = deleted ? null : publicClaimKey(subject, readClaim(subject));
    sendJson(res, 200, { eligible: tutorial.eligible, deleted, channel: `@${CHANNEL_USERNAME}`, ...(existing ? { key: existing.key, claimedAt: existing.claimedAt } : {}) });
    return true;
  }
