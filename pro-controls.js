...
function safeExport(uid) {
  ...
  return {
    ...
    message: { text: settings.adMessage || "", entities: settings.adEntities || [] },
    destinations: (settings.groups || []).map(group => ({ label: group.label || "", type: group.type || "", username: group.username || "" })),
    intervalMinutes: Number(settings.intervalMinutes || 30),
    pro: {
      placeholders: pro.placeholders,
      staggerSeconds: pro.staggerSeconds,
      schedule: pro.schedule,
      templates: pro.templates.map(template => ({ ...template, media: undefined })),
    },
  };
}
...
function registerHandlers(bot) {
  ...
  bot.callbackQuery("export_config", async ctx => {
    const uid = uidOf(ctx); await ctx.answerCallbackQuery({ text: "Preparing export…" });
    const buffer = Buffer.from(JSON.stringify(safeExport(uid), null, 2), "utf8");
    await ctx.replyWithDocument(new InputFile(buffer, "telepilot-config.json"), { caption: "📦 TelePilot configuration export\n\nSessions, access keys and login credentials are never included." });
  });
  bot.callbackQuery("import_config", async ctx => {
    const uid = uidOf(ctx); await ctx.answerCallbackQuery(); proAwaiting.set(uid, { type: "import", messageId: ctx.callbackQuery.message?.message_id });
    await ctx.editMessageText("📥 Import configuration\n\nSend a TelePilot configuration JSON file.\n\nPersonal sessions and access keys are never imported.", { reply_markup: new InlineKeyboard().text("Cancel", "tools") });
  });
  ...
  bot.on(["message:photo", "message:video", "message:animation", "message:document"], async ctx => {
    ...
    if (pending?.type === "import" && ctx.message.document) {
      proAwaiting.delete(uid);
      try {
        if (Number(ctx.message.document.file_size || 0) > 1_000_000) throw new Error("Configuration file is too large.");
        ...
        const response = await fetch(`https://api.telegram.org/file/bot${BOT_TOKEN}/${file.file_path}`);
        if (!response.ok) throw new Error("Could not download configuration file.");
        const parsed = JSON.parse(await response.text());
        if (parsed?.kind !== "TelePilotConfig" || Number(parsed?.version) !== 1) throw new Error("That is not a supported TelePilot configuration export.");

        const importedPro = readProSettings(uid);
        if (parsed.pro?.schedule) importedPro.schedule = { ...importedPro.schedule, ...parsed.pro.schedule };
        if (typeof parsed.pro?.placeholders === "boolean") importedPro.placeholders = parsed.pro.placeholders;
        if ([0,2,5,10,20].includes(Number(parsed.pro?.staggerSeconds))) importedPro.staggerSeconds = Number(parsed.pro.staggerSeconds);
        if (Array.isArray(parsed.pro?.templates)) importedPro.templates = parsed.pro.templates.slice(0, 20);
        writeProSettings(uid, importedPro);

        if (parsed.message?.text) await applyMessage(ctx, String(parsed.message.text), Array.isArray(parsed.message.entities) ? parsed.message.entities : []);
        if (Number.isFinite(Number(parsed.intervalMinutes))) await applyInterval(ctx, Number(parsed.intervalMinutes));
        let added = 0;
        for (const destination of Array.isArray(parsed.destinations) ? parsed.destinations.slice(0, 50) : []) {
          if (!destination?.username) continue;
          try { await applyDestination(ctx, destination.username); added++; } catch {}
        }
        try { await ctx.api.deleteMessage(ctx.chat.id, ctx.message.message_id); } catch {}
        await ctx.api.editMessageText(ctx.chat.id, pending.messageId, `📥 Import complete\n\n✅ Message/settings imported\n✅ ${added} public destination${added === 1 ? "" : "s"} processed\n\nPrivate destinations must be re-added with /addhere.`, { reply_markup: new InlineKeyboard().text("⬅️ Dashboard", "home") });
      } catch (err) {
        await ctx.api.editMessageText(ctx.chat.id, pending.messageId, `📥 Import configuration\n\n❌ ${String(err?.message || err).slice(0, 180)}`, { reply_markup: new InlineKeyboard().text("Try again", "import_config").row().text("⬅️ Tools", "tools") });
      }
      return;
    }
