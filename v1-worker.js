...
function savedInputPeer(destination){const id=String(destination?.id||"").trim(),accessHash=String(destination?.accessHash??"").trim();if(/^-100\d+$/.test(id)&&/^-?\d+$/.test(accessHash)&&accessHash){return new MtApi.InputPeerChannel({channelId:bigInt(id.slice(4)),accessHash:bigInt(accessHash)});}if(/^-(?!100)\d+$/.test(id)){return new MtApi.InputPeerChat({chatId:bigInt(id.slice(1))});}return null;}
